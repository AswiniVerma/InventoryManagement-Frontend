...
const LostToolList = () =>{
  ...
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        ...
        axios.get(api_endpoints.getAllLostTool)
            .then(response => {
                ...
                <tbody>
                    {tools.map(tool => (
                        <tr key={tool._id}>
                            ...
                            <Link className=" m-2 small-button" to={`/toolFound/${tool.id}/${tool.name}/${tool.quantity}`} >Update Lost Tool Details</Link> 
                            </td>
